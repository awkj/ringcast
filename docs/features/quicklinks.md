...
Quicklinks reuse RingCast's one template engine — the same
...
RingCast's own dialog and leaves no partial state.
...
independently selectable category. RingCast reads `name`, `link`, `createdAt` and the optional
