...
Debug (`io.github.awkj.ringcast.dev`), beta, and stable therefore never share snippet files. The storage
...
frontmatter keeps the same identity. Renaming a file outside RingCast appears as deletion of the old
...
an independently selectable category. RingCast reads `title`, `text` and the optional `keyword` from
...
suffixes as snippets created in RingCast, and duplicate keywords are preserved.
...
there are no aliases, so a key RingCast does not know names itself in the error.
...
A token RingCast cannot parse — an unknown name, an unknown modifier, a duplicated or unsupported
...
duplicates resolve by file identity. RingCast-tagged synthetic events are ignored.
...
After either launcher or keyword delivery is confirmed, RingCast may show a brief non-activating,
...
for permissions or activates RingCast. Failed, cancelled, rejected, or prompt-cancelled expansions do
...
Longer or multiline fallback text uses a temporary paste. RingCast snapshots every item, type and data
payload, then **lends a board holding nothing but the expansion** — one item carrying the plain text
and RingCast's own marker type — and restores by rewriting the snapshot whole.
...
previous copy instead. A representation RingCast cannot rewrite to mean the expansion is one it must
...
synchronizes to RingCast's ownership changes so temporary or restored text is not added as new history.
...
any pasteboard restoration still owned by RingCast.
...
non-ignored input to `cancelAutomaticExpansion`, and RingCast's own tagged synthetic events classify
...
between revalidation and mutation, so RingCast does not claim that impossible guarantee.
