...
Export and import of RingCast's own data as a single `.ringcast` file, plus the entry point for
...
- **No absolute path may enter a `.ringcast`.** A clip's `imagePath` names a file on the Mac that wrote
...
| `Model/BackupArchive.swift` | Directory ⇄ `.ringcast`; the only file importing `AppleArchive` |
...
| `Service/RaycastImportReader.swift` | Raycast → RingCast field mapping |
...
The old flat `RingCast-Settings-*.json` export is gone rather than deprecated, and nothing reads it.
