...
enum UninstallProtection: String, Hashable, Sendable, CaseIterable {
    ...
    var lockReason: String? {
        ...
        case .needsFullDiskAccess:
            return "Needs Full Disk Access, which \(AppIdentity.name) doesn’t request. "
                + "Grant it in System Settings › Privacy & Security to include this item."
        case .parentNotWritable:
            return "Its enclosing folder isn’t writable by you, and \(AppIdentity.name) never asks for an "
